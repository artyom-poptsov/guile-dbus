...
{
    struct gdbus_pending_call* data = _scm_to_gdbus_pending_call(obj);
...

struct gdbus_pending_call* make_dbus_pending_call_data()
{
    return scm_gc_malloc(sizeof(struct gdbus_pending_call),
...
{
    SCM smob;
    struct gdbus_pending_call* gdbus_call = make_dbus_pending_call_data();
...
struct gdbus_pending_call* _scm_to_gdbus_pending_call(SCM x)
{
    ...
    return (struct gdbus_pending_call *) SCM_SMOB_DATA(x);
}
...
void init_dbus_pending_call_type()
{
    gdbus_pending_call_tag
        = scm_make_smob_type(GDBUS_PENDING_CALL_TYPE_NAME,
                             sizeof(struct gdbus_pending_call));
